	...
	vector<RigidTransform4d>& localTransforms, vector<RigidTransform4d>& globalTransforms)
{
	...
	{
		double RJointOrient[9], RChild[9];
		euler2Rotation(jointOrientationEulerAngles[i], RJointOrient, RotateOrder::XYZ);
		euler2Rotation(eulerAngles[i], RChild, rotateOrders[i]);
		Mat3d MRJointOrient(RJointOrient), MRChild(RChild);
		Mat3d MRLocal(MRJointOrient * MRChild);
		localTransforms[i] = RigidTransform4d(MRLocal, translations[i]);
	}
	...
	{
		int curJointIndex = jointUpdateOrder[i];
		int curJointParentIndex = jointParents[curJointIndex];
		if (curJointParentIndex == -1)
		{
			// root
			globalTransforms[curJointIndex] = localTransforms[curJointIndex];
		}
		else
		{
			globalTransforms[curJointIndex] = globalTransforms[curJointParentIndex] * localTransforms[curJointIndex];
		}
	}
